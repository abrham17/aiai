...
export function ChallengeCanvas({ challenge, onComplete }: ChallengeCanvasProps) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const lossElement = document.querySelector('.min-loss-val');
      if (lossElement && lossElement.textContent) {
        const val = parseFloat(lossElement.textContent);
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', margin: 0, lineHeight: 1.5 }}>
              You navigated the treacherous loss landscape and found the global minimum. Your adaptive strategies are world-class!
            </p>
          </div>
        </div>
      )}
